...
namespace Serilog.Sinks.RawFile.Tests
{
    public class PathRollerTests
    {
        ...
        {
            var roller = new PathRoller(Path.Combine("Logs", "log-.txt"), RawFileRollingInterval.Day);
        ...
        {
            var roller = new PathRoller(Path.Combine("Logs", "log-"), RawFileRollingInterval.Day);
            var now = new DateTime(2013, 7, 14, 3, 24, 9, 980);
            roller.GetLogFilePath(now, null, out var path);
        ...
        {
            var roller = new PathRoller("log-", RawFileRollingInterval.Day);
            var now = new DateTime(2013, 7, 14, 3, 24, 9, 980);
            roller.GetLogFilePath(now, null, out var path);
        ...
        {
            var roller = new PathRoller("log-.txt", RawFileRollingInterval.Day);
        ...
        [Theory]
        [InlineData("log-.txt", "log-20131210.txt", "log-20131210_031.txt", RawFileRollingInterval.Day)]
        [InlineData("log-.txt", "log-2013121013.txt", "log-2013121013_031.txt", RawFileRollingInterval.Hour)]
        public void MatchingSelectsFiles(string template, string zeroth, string thirtyFirst, RawFileRollingInterval interval)
        {
        ...
        [Theory]
        [InlineData("log-.txt", "log-20150101.txt", "log-20141231.txt", RawFileRollingInterval.Day)]
        [InlineData("log-.txt", "log-2015010110.txt", "log-2015010109.txt", RawFileRollingInterval.Hour)]
        public void MatchingParsesSubstitutions(string template, string newer, string older, RawFileRollingInterval interval)
        {
            var roller = new PathRoller(template, interval);
